...
function showError(msg){
  ...
  const card = document.querySelector('.card')
  if(card){ card.classList.add('has-error'); card.classList.remove('no-error') }
}
function hideError(){
  errorBox.classList.add('hidden');
  errorBox.textContent = ''
  const card = document.querySelector('.card')
  if(card){ card.classList.remove('has-error'); card.classList.add('no-error') }
}
...
// initialize card state based on whether an error is visible
document.addEventListener('DOMContentLoaded', ()=>{
  const card = document.querySelector('.card')
  const errorBox = document.getElementById('error')
  if(card){
    if(errorBox && !errorBox.classList.contains('hidden') && errorBox.textContent.trim()){
      card.classList.add('has-error')
    } else {
      card.classList.add('no-error')
    }
  }
})